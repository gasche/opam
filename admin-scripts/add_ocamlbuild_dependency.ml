      ...
      OpamParallel.iter
        ...
        ~command:(fun package ->
            ...
            if in_blacklist package then Done ()
              ...
              | `Error () ->
                Printf.printf "failed to download %S\n%!" package_str;
                Done ()
              | `Successful None ->
    ...
    iter_packages
      ~f:(fun package prefix opam ->
          ...
          | true ->
            ...
            OpamGlobals.skip_version_checks := true;
            (* The reason for skipping version checks is that,
               otherwise, OPAM.write enables some additional content
               checks when the package version is 1.2, which warn and
               rewrite the 'depopts' field that contain version
               constraints. Apparently the internal "of_syntax"
               function supports a ~conservative flag to disable this
               zealous behavior, but it is not exposed to API users;
               disabling version checks is the next best thing. *)
